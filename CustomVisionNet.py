...
class VisionNetworkWithPooling(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space: gym.spaces.Space,
        ...
        for out_channels, kernel, stride in filters[:-1]:
            ...
            in_size = (out_size[0]//2, out_size[1]//2)
        ...
        # `num_outputs`), then add another linear one of size `num_outputs`.
            ...
            if num_outputs:
                ...
                padding, dummy_output = same_padding(in_size, [1, 1], [1, 1])
                #for _ in range(1000):
                #        print(dummy_output)
                if post_fcnet_hiddens:
                    layers.append(nn.Flatten())
                    #in_size = out_channels
                    in_size = model_config["custom_model_config"]["post_fcnet_inputsize"]
                    #for _ in range(1000):
                    #    print(out_channels)
                    #    print(out_size)
                    # Add (optional) post-fc-stack after last Conv2D layer.
