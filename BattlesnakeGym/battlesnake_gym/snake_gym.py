...
from battlesnake_gym import rewards

class BattlesnakeGym(MultiAgentEnv):
    metadata = {
    ...
    def __init__(self, config=None):
        ...
        self.number_of_snakes = config["number_of_snakes"]
        self.initial_game_state = config ["initial_game_state"]
        self.snake_spawn_locations = config["snake_spawn_locations"]
        self.food_spawn_locations = config["food_spawn_locations"]

        #self.action_space = MultiAgentActionSpace(
        #    [spaces.Discrete(4) for _ in range(self.number_of_snakes)])
    ...
    def reset(self, map_size=None):
        ...
        return self._get_observations() #, {}, dones, info
    ...
    def step(self, actions, episodes=None):
        ...
        # setup reward dict
        reward = {}
        snake_info = {i: {} for i in range(self.number_of_snakes)}
        ...
        for i, snake in enumerate(self.snakes.get_snakes()):
            ...
            if snake.health == 0:
                ...
                snake_info[i]["snake_info"] =  "Starved"
                continue
            ...
            if is_forbidden:
                ...
                snake_info[i]["snake_info"] = "Forbidden move"
        ...
        for i, snake in enumerate(self.snakes.get_snakes()):
            ...
            snake_info[i]["snake_info"] = outcome
        ...
        for i, snake in enumerate(self.snakes.get_snakes()):
            if "snake_info" not in snake_info[i]:
                snake_info[i]["snake_info"] = "Dead"
                del snake_alive_dict[i]
                del reward[i]
            snake_info[i]["snake_health"] = snake.health
            snake_info[i]["current_turn"] = self.turn_count
            snake_info[i]["snake_max_len"] = self.snake_max_len[i]
            if snake_info[i]["snake_info"] == "Dead":
                del snake_info[i]
                del obs[i]
            if snake.is_alive():
